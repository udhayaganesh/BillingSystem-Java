...
function getTableObj(tableid)
{
	...
	{	
		...
		{
			alert('1'+document.getElementsByTagName('table').item(i));
		...
		{	 
			...
			if(temp1==0)
			{
				if(colref.getElementsByTagName('input').item(0).getAttribute("id").substring(0,1)=="d" || colref.getElementsByTagName('input').item(0).getAttribute("name").substring(0,1)=="d")
				{
					...
					// Premnath-chn-06/06/2007-beg
					//var a="gridAnchor"+j+curr_rec_num;
					var a="gridAnchor"+tableid+j+curr_rec_num;
					// Premnath-chn-06/06/2007-End		
					//C.Premnath - 0920 - 29-06-2007-beg 			
					//var click="cal3.select(document.all."+txtFldId+curr_rec_num+",'"+a+"','DD-MM-YYYY'); return false;";
...
function deleteRowGrid(tableid,whichRow)
{
	...
	{	
		...
		{
			var colref=myrow.getElementsByTagName('td').item(j);
			//kaviarasu-chn-22/6/2007-beg changes for deleting combobox
			//kaviarasu-chn-29/6/2007-beg
			//if(colref.childNodes(0).getAttribute("name").substring(0,3)!="cbo")
			//{  kaviarasu-chn-29/6/2007-end			
			//kaviarasu-chn-22/6/2007-end changes for deleting combobox
			if (colref.getElementsByTagName('select').item(0)== null){//LSudhakar-chn-03/07/2007 for deleting combobox		
				if(colref.getElementsByTagName('input').item(0).getAttribute("id").substring(0,1)=="d" || colref.getElementsByTagName('input').item(0).getAttribute("name").substring(0,1)=="d")
				{
					var txtFldId=colref.getElementsByTagName('input').item(0).getAttribute("id");
					txtFldId=txtFldId.substring(0,txtFldId.length-1);
			...
			}//LSudhakar-chn-03/07/2007 for deleting combobox
		}
	}
...
function setCellValue(tableid,rownum,colnum,value)
{
	...
	// S.Sathishkumar 0796 start
	if(colref=="null" || colref==null || colref=="undefined") 
			return;
	// S.Sathishkumar 0796 end			
		...
		else
		{
			switch(colref.getElementsByTagName('input').item(0).getAttribute("type"))
			{
				case "text":
					...
					break;
				case "checkbox":
//					if(value=="" || value=="N" || value=="0"||value=="null" || value==null)  S.Sathishkumar 0796 25-04-2007
